...
Run all Phase 2–7 PHP test scripts, lint every PHP file, and run `git diff --check`. Phase 7 focused checks cover independent duplicate-seed plantings, failed-create form retention, chronology validation, method-aware germination and supported/unsupported transplant/harvest calculations, transplant-date forward rolling, actual-date precedence, strict winter-month parsing, winter eligibility isolation, unknown research, December/January handling, Winter Sown integration, recurring frost boundaries, identifiable unavailable cards, preservation of existing setting keys, version-2 and legacy version-1 backup validation, research-preserving duplication, and POST/CSRF route wiring.

On a migrated database, create multiple records for one seed and exercise every My Garden section/status. Verify readable garden dates, clear Needs Attention reasons, malformed/foreign seed IDs, invalid dates and chronology, invalid enums, zero/oversized quantities, and overlong text. Verify GET or invalid-CSRF writes fail. Confirm deleting a referenced seed is blocked rather than silently deleting planting history. Test explicit and missing research in all four winter months and confirm only `Suitable` plus an exact explicitly selected month appears. Eligible seed cards should remain compact and must not display stored citation text; citations remain available in the individual Structured Research Fields editor. Confirm Winter Sown creates a normal independent My Garden planting preselected with the correct seed and method. Duplicate a researched seed and confirm its research is copied while an ordinary seed edit preserves it. Restore disposable version-2 and pre-Phase-7 version-1 fixtures and confirm the latter creates no planting history and defaults missing research fields. Unknown research must remain ineligible until explicitly researched. At phone and desktop widths, verify cards/forms remain usable and calculated labels cannot be confused with actual dates.
