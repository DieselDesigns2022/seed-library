...
## Phase 7 implementation pass 1 verification
...
On a disposable migrated database, create multiple records for one seed and exercise every My Garden section/status. Verify malformed/foreign seed IDs, invalid dates and chronology, invalid enums, zero/oversized quantities, and overlong text are rejected without replacing submitted form values. Verify GET or invalid-CSRF writes fail. Confirm deleting a referenced seed is blocked rather than silently deleting planting history. Test explicit and missing research in all four winter months and confirm only `Suitable` plus an exact explicitly selected month appears. Duplicate a researched seed and confirm its research is copied while an ordinary seed edit preserves it. Restore disposable version-2 and pre-Phase-7 version-1 fixtures and confirm the latter creates no planting history and defaults missing research fields. Confirm existing records remain Unknown until researched; this pass intentionally contains no seed-by-seed winter-sowing population. At phone and desktop widths, verify cards/forms remain usable and calculated labels cannot be confused with actual dates.
