...
{
    return "(({$alias}.planting_start_month IS NOT NULL AND {$alias}.planting_end_month IS NOT NULL) AND (({$alias}.planting_start_month <= {$alias}.planting_end_month AND ? BETWEEN {$alias}.planting_start_month AND {$alias}.planting_end_month) OR ({$alias}.planting_start_month > {$alias}.planting_end_month AND (? >= {$alias}.planting_start_month OR ? <= {$alias}.planting_end_month))))";
}
...
{
    return ['seed_number','name','variety','category_id','plant_family_id','status_id','plant_type','planting_method','days_to_germination_min','days_to_germination_max','days_to_maturity','planting_start_month','planting_start_day','planting_end_month','planting_end_day','indoor_start_weeks_before_frost','transplant_weeks_after_frost','succession_days','sun_requirements','water_requirements','soil_requirements','spacing','sowing_depth','plant_height','container_friendly','pollinator_friendly','medicinal','perennial','frost_tolerant','heat_tolerant','trellis_needed','seed_source','packet_year','quantity','purchase_date','expiration_year','notes'];
}
