...
function calendar_page(): void
{
    ...
    <div class="card"><div class="card-header fw-bold"><?=e($group!==''?$rules[$group].' · ':'')?><?=e(month_name($month))?> (<?=count($seeds)?>)</div><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Seed #</th><th>Seed</th><th>Category</th><th>Method</th><th>Start</th><th>Last</th><th>Maturity</th><th>Notes</th><th></th></tr></thead><tbody><?php foreach($seeds as $seed):?><tr><td><?=e($seed['seed_number'])?></td><td><strong><?=e($seed['name'])?></strong></td><td><?=e($seed['category_name']?:'—')?></td><td><?=e($seed['planting_method']?:'—')?></td><td><?=e(date_label($seed['planting_start_month'],$seed['planting_start_day']))?></td><td><?=e(date_label($seed['planting_end_month'],$seed['planting_end_day']))?></td><td><?=e(maturity_display($seed,'—'))?></td><td><?php if(($seed['notes']??'')!==''):?><details><summary class="small fw-semibold text-success" style="cursor:pointer">View Notes</summary><div class="small mt-2 p-2 bg-light border rounded text-start" style="min-width:18rem;max-width:32rem;white-space:normal;overflow-wrap:anywhere"><?=nl2br(e($seed['notes']))?></div></details><?php else:?>—<?php endif?></td><td class="text-nowrap"><a class="btn btn-sm btn-outline-success" href="<?=e(url('seeds/'.$seed['id']))?>">View Seed</a></td></tr><?php endforeach;if(!$seeds):?><tr><td colspan="9" class="text-muted">No seeds match this month and group.</td></tr><?php endif?></tbody></table></div></div><?php });
}
