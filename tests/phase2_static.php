...
$checks=[
 'quick search is limited to seed number, name, and variety'=>str_contains($seeds,"(s.seed_number LIKE ? OR s.name LIKE ? OR s.variety LIKE ?)")&&!str_contains($seeds,"s.notes LIKE ?")&&!str_contains($seeds,"c.name LIKE ? OR pf.name LIKE ?"),
 'advanced relationship and use filters still use relationship subqueries'=>str_contains($seeds,'EXISTS (SELECT 1 FROM companion_relationships')&&str_contains($seeds,'SELECT COUNT(DISTINCT su.use_id)'),
 ...
 'empty result, simplified clear controls, and responsive layouts exist'=>str_contains($inventory,'none match the current search and filters')&&str_contains($inventory,'Clear All')&&str_contains($inventory,'Quick Search')&&str_contains($inventory,'More Filters')&&str_contains($inventory,'mobile-card'),
